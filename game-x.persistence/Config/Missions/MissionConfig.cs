using System.Text.Json;
using game_x.domain.Entities.Missions;
using game_x.domain.ValueObjects.Missions;
using Microsoft.EntityFrameworkCore.ChangeTracking;
using Microsoft.EntityFrameworkCore.Metadata.Builders;

namespace game_x.persistence.Config.Missions;

public sealed class MissionConfig : IEntityTypeConfiguration<Mission>
{
    public void Configure(EntityTypeBuilder<Mission> b)
    {
        b.ToTable("missions");

        b.HasKey(x => x.Id);

        #region Properties
        b.Property(x => x.Id)
            .HasColumnName("id")
            .ValueGeneratedOnAdd();

        b.Property(g => g.PublicId)
            .HasColumnName("public_id")
            .IsRequired()
            .HasDefaultValueSql("gen_random_uuid()");
        
        b.Property(x => x.RewardConfigId)
            .HasColumnName("reward_config_id");
        
        b.Property(x => x.Code)
            .HasColumnName("code")
            .IsRequired()
            .HasMaxLength(128);

        b.Property(x => x.Type)
            .HasColumnName("type")
            .HasConversion<string>()
            .IsRequired();

        b.Property(x => x.Title)
            .HasColumnName("title")
            .IsRequired()
            .HasMaxLength(256);

        b.Property(x => x.Description)
            .HasColumnName("description")
            .IsRequired(false)
            .HasMaxLength(4096);

        b.Property(x => x.ResetType)
            .HasColumnName("reset_type")
            .IsRequired(false)
            .HasConversion<string>();
        
        b.Property(x => x.IsActive)
            .HasColumnName("is_active")
            .IsRequired()
            .HasDefaultValue(true);
        
        b.Property(x => x.ResetType)
            .HasColumnName("reset_type")
            .IsRequired(false)
            .HasConversion<string>();
        
        b.Property(x => x.ConfigData)
            .HasColumnName("config")
            .HasColumnType("jsonb")
            .IsRequired()
            .HasConversion(
                v => JsonSerializer.Serialize(v, JsonSerializerOptions.Default),
                v => JsonSerializer.Deserialize<MissionConfigData>(
                    v,
                    JsonSerializerOptions.Default
                )!
            )
            .Metadata.SetValueComparer(
                new ValueComparer<MissionConfigData>(
                    (l, r) => l == r,
                    v => v.GetHashCode(),
                    v => JsonSerializer.Deserialize<MissionConfigData>(
                        JsonSerializer.Serialize(v, JsonSerializerOptions.Default),
                        JsonSerializerOptions.Default
                    )!
                )
            );

        b.Property(x => x.StartAt)
            .HasColumnName("start_at")
            .HasColumnType("timestamp with time zone");

        b.Property(x => x.EndAt)
            .HasColumnName("end_at")
            .HasColumnType("timestamp with time zone");
        
        b.Property(x => x.DeletedAt)
            .HasColumnName("deleted_at")
            .HasColumnType("timestamp with time zone");
        #endregion

        #region Indexes
        b.HasIndex(x => x.PublicId)
            .IsUnique()
            .HasDatabaseName("ux_missions_public_id");
        
        b.HasIndex(x => x.Code)
            .IsUnique()
            .HasDatabaseName("ux_missions_code")
            .HasFilter("deleted_at IS NULL");

        b.HasIndex(x => new { x.Type, x.IsActive })
            .HasDatabaseName("ix_missions_type_active");

        b.HasIndex(x => new { x.StartAt, x.EndAt })
            .HasDatabaseName("ix_missions_date_range");
        #endregion

        #region Relationship
        b.HasOne(x => x.RewardPool)
            .WithMany(x => x.Missions)
            .HasForeignKey(x => x.RewardConfigId)
            .OnDelete(DeleteBehavior.SetNull);
        #endregion
    }
}