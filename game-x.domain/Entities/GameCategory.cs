...
public sealed class GameCategory : BaseEntity<int>, IAuditable
{
    ...
    public ICollection<GameCategoryTranslation> Translations { get; private set; } = [];
    ...
    public void UpsertTranslation(LanguageCode lang, string name, string description, string note)
    {
        ...
        if (existing is null)
        {
            var newTranslation = GameCategoryTranslation.Create(Id, lang, name, description, note);
