...
public sealed class OnOrderApprovedHandler(
    IUnitOfWork unitOfWork,
    IUserRepo userRepo,
    INotificationRepo notificationRepo,
    IStoreHubService storeHubService,
    IAdminHubService adminHubService,
    IClientHubService clientHubService) : IApplicationEventHandler<OnOrderApprovedEvent>
{
    public async Task Handle(OnOrderApprovedEvent @event, CancellationToken ct = default)
    {
    ...
    private async Task SendToStaff(Order order, CancellationToken ct)
    {
        ...
        var notification = Notification.Create(
            NotificationMessageKey.Order_Approved,
            order.StaffId,
