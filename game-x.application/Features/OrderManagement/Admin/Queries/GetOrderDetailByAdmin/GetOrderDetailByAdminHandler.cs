...
public sealed class GetOrderDetailByAdminHandler(OrderMapper orderMapper,
    IOrderRepo orderRepo,
    IUxmService uxmService,
    IAsymmetricCryptoService asymmetricCryptoService,
    IOptions<GalaxySettings> galaxySettings,
    IAsymmetricKeyCacheService  asymmetricKeyCacheService) : IQueryHandler<GetOrderDetailByAdminQuery, OrderDetailInfoDto>
{
    public async Task<OrderDetailInfoDto> Handle(GetOrderDetailByAdminQuery request, CancellationToken ct = default)
    {
    ...
    private Task<SecureRequest<GetUxmOrderDetailInfoRequest>> CreateUxmRequest(GetOrderDetailByAdminQuery request, string orderUid)
    {
