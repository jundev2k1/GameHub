...
public sealed class ReviewOrderByAdminCommandHandler(
    IUnitOfWork unitOfWork,
    IOrderRepo orderRepo,
    IApplicationEventDispatcher eventDispatcher)
    : ICommandHandler<ReviewOrderByAdminCommand>
{
    public async Task<Unit> Handle(ReviewOrderByAdminCommand request, CancellationToken ct = default)
    {
        ...
        await unitOfWork.WithTransactionAsync(
            async () =>
            {
                await orderRepo.UpdateAsync(request.OrderId, updatedOrder => updatedOrder.UpdateStatus(OrderStatus.Of(request.OrderStatus)), ct);
            }, ct);
