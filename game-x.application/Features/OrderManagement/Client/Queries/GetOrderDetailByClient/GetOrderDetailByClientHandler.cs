...
public sealed class GetOrderDetailByClientHandler(
    OrderMapper orderMapper,
    IOrderRepo orderRepo,
    IUxmService uxmService,
    IAsymmetricCryptoService asymmetricCryptoService,
    IOptions<GalaxySettings> galaxySettings,
    IAsymmetricKeyCacheService  asymmetricKeyCacheService) : IQueryHandler<GetOrderDetailByClientQuery, OrderDetailInfoDto>
{
    public async Task<OrderDetailInfoDto> Handle(GetOrderDetailByClientQuery request, CancellationToken ct = default)
    {
    ...
    private Task<SecureRequest<GetUxmOrderDetailInfoRequest>> CreateUxmRequest(GetOrderDetailByClientQuery request, string orderUid)
    {
