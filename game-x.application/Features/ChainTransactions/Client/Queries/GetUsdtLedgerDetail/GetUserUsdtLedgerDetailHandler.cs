...
public sealed class GetUserUsdtLedgerDetailHandler(
    ...
    : IQueryHandler<GetUserUsdtLedgerDetailQuery, UserUsdtLedgerDto>
{
    public async Task<UserUsdtLedgerDto> Handle(GetUserUsdtLedgerDetailQuery request, CancellationToken ct = default)
    {
        var userId = userAccessor.GetUserId();
        var result = await userUsdtLedgerRepo.GetDetailByUserAsync(userId, request.UsdtLedgerId, ct);
        
        return result.Adapt<UserUsdtLedgerDto>();
    }
}